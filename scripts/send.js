...
function setDificuldade(dificuldade) {
  ...
  document.getElementById("extra_dificil").disabled = "disabled";
...
function preparaWave() {
  if (geracao == 0) {
    ...
    let waveMaximaAtingida = Math.max.apply(
      null,
      genotipoDaRodada.map((item) => item.wave)
    );
    if (wave <= waveMaximaAtingida) {
      console.log(
        "Nessa rodada tentaremos posicionar",
        gunsDaRodada.length,
        "torres."
      );
...
function variarPorQuantidade(xy) {
  ...
  let finalDirection = [
    xy[0] + directions[direction][0],
    xy[1] + directions[direction][1],
  ];

  while (
    finalDirection[0] < 0 ||
    finalDirection[0] > limiteX ||
    finalDirection[1] < 0 ||
    finalDirection[1] > limiteY
  ) {
...
function salvarDados() {
  ...
  document.getElementById("myTextarea").value =
    JSON.stringify(novosIndividuos) +
    "&" +
    JSON.stringify(todosIndividuosDeTodasGeracoes);
...
function geraNovaGeracao() {
  ...
  const individuosOrdenadosPorWaveMaxima = sortObj(
    novosIndividuos,
    "ultimaWaveAtingida"
  );
  ...
  console.log(
    "individuosOrdenadosPorWaveMaxima",
    individuosOrdenadosPorWaveMaxima
  );
  ...
  while (melhoresIndividuos.length < quantidadeMelhoresIndividuos) {
    for (
      i = 0;
      i < individuosOrdenadosPorWaveMaxima.length &&
      melhoresIndividuos.length < quantidadeMelhoresIndividuos;
      i++
    ) {
  ...
  for (let i = 0; i < melhoresIndividuos.length; i++) {
    for (let j = 0; j < melhoresIndividuos.length; j++) {
      if (i != j) {
        ...
        filho = mutateInto(
          melhoresIndividuos[i].torresPosicionadas,
          melhoresIndividuos[j].torresPosicionadas,
          0.1
        );
